...
- `~/.gemini/history/`、设置文件、工作区运行态、sqlite 文件
- 当前生效设置、缓存和运行态
- Gemini 必须直接拥有的原生运行文件
...
- 如果 Gemini 为了启动保留本地规则或入口文件，把它当成本地投影副本，不当成正式来源。
- Gemini CLI 目前不支持本地 hooks（包括 `PreToolUse` / `PostToolUse`）；本地长期文档改动后，手动登记 `handoff/sync-queue.md`，再判断是否回写共享层。
- 运行态噪音、sqlite/jsonl 状态、缓存和工作区运行态继续留在本地。
...
- 用户要求安装 skill / 技能 / GitHub skill repo 且未明确指定单一模型时，默认安装到 `~/.agents/skills/` 并验证跨模型可见；不要停在 Gemini 单端目录。
- 只有用户明确说 Gemini-only，或该 skill 是 Gemini 工具原生例外时，才写 Gemini 自有 home。细则见 `rules/skills-governance.md`。
...
- 新的本地文件只有在内容属于运行态，或 Gemini 明确要求本地投影副本时才成立。
